    ...
    models_to_train = [
        {
            ...
            # 加载预训练的 MobileNetV2 模型（在 ImageNet 上预训练）
            ...
            # 修改模型的分类头（最后一层）
            # 原始 MobileNetV2 的 classifier 是：Dropout -> Linear(1280, 1000)
            # 这里替换为：Dropout(0.2) -> Linear(1280, 2)
            # 因为我们的任务是二分类（健康 vs 病害）
        },
        {
            'name': 'ResNet18',
            'model': models.resnet18(pretrained=True),
            # 加载预训练的 ResNet18 模型
            'modify_head': lambda m: setattr(m, 'fc', nn.Linear(m.fc.in_features, 2))
            # ResNet18 的分类头是 fc（全连接层），原始是 (512, 1000)
            # 替换为 (512, 2) 适应二分类任务
        }
    ]
